...
export default function RoleList({ roles, onEdit, onDelete, isLoading }: RoleListProps) {
    if (isLoading) {
        return (
            ...
            {roles.map((role) => (
                ...
                >
                    <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                        ...
                        {role.isSystem && (
                            <span className="text-xs text-muted-foreground bg-secondary px-2 py-1 rounded">System</span>
                        )}
                    </div>
                    ...
                    <p className="text-sm text-muted-foreground mb-4 line-clamp-2">{role.description || 'No description'}</p>
